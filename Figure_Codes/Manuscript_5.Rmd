...
```{r read_output_diann}
Main_output <- read_delim('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Proteomics/Melanoma/DIANN/Main_Report.tsv', delim = '\t', col_names = TRUE)
head(Main_output)
...
```{r read_MDTs}
Switches_Melanoma <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Melanoma/Switches/CorrectedStat/Switches_TuPro_GTEx.RDS')
```
...
Main_output_filtered_dMDT <- Main_output_filtered[Main_output_filtered$Protein.Ids %in% Switches_Melanoma_ensps$Protein.stable.ID, ]
...
```{r mdt_enst_merging}
Switches_Melanoma_MDTS_splitted <- Switches_Melanoma %>%
  mutate(MDT_GTEx = strsplit(as.character(MDT_GTEx), ",")) %>%
  unnest(MDT_GTEx)

Switches_Melanoma_ensps_mdts <- merge(ensg_enst_ensp_apris_v104, Switches_Melanoma_MDTS_splitted, by.x= 'Transcript.stable.ID', by.y = 'MDT_GTEx', sort=FALSE)
head(Switches_Melanoma_ensps_mdts)
```


```{r upper_code_vectorizaition}
library(data.table)
dMDT_vector <- unique(Switches_Melanoma_ensps$Protein.stable.ID)
MDT_vector <- unique(Switches_Melanoma_ensps_mdts$Protein.stable.ID)
...
library(purrr)
include_ids <- unique(Switches_Melanoma_ensps$Protein.stable.ID)
exclude_ids <- unique(Switches_Melanoma_ensps_mdts$Protein.stable.ID)
...
Main_output_filtered_dMDT_exc_MDT_exc_ClassI_sepRows <- Main_output_filtered_dMDT_exc_MDT_exc_ClassI %>% separate_rows(Protein.Ids, sep = ";") %>% dplyr::filter(Protein.Ids %in% Switches_Melanoma_ensps$Protein.stable.ID) %>% dplyr::filter(!Protein.Ids %in% Main_output_filtered_dMDT$Protein.Ids)  # split the protein Ids and find cMDTs that are Class II - 164 Protein Ids found with Class II strategy  - Not Sample Specific!!
```


```{r focus_only_on_unique_peptides}
dMDT_proteomics_dt_unique_peptide <- dMDT_proteomics_dt[dMDT_proteomics_dt$Protein.Ids %in% Switches_Melanoma_ensps$Protein.stable.ID, ] # 121 - dMDT specific peptides are counted.

MDT_proteomics_dt_unique_peptide <- MDT_proteomics_dt[MDT_proteomics_dt$Protein.Ids %in% Switches_Melanoma_ensps_mdts$Protein.stable.ID, ] # unique MDT peptides - 311 MDT peptides are found in the samples. 

Swicthes_in_Dataset_gtex_tp_mel <- Switches_Melanoma_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n))
```

## length(unique(dMDT_proteomics_dt_unique_peptide$Protein.Ids)) 132 isoform has dMDT-specific peptides
## length(unique(MDT_proteomics_dt_unique_peptide$Protein.Ids)) 464 MDT isoforms has MDT-specific peptides in the samples
...
#rna_each_sample_output <- Switches_Melanoma_ensps[,c(3,9)] %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()

## Class I isoforms in the RNA Samples
rna_each_sample_output_3 <- Switches_Melanoma_ensps[,c(3,9)] %>% dplyr::filter(Protein.stable.ID %in% Main_output_filtered_dMDT$Protein.Ids) %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()

## Class II isoforms in the RNA samples
rna_each_sample_output_4 <- Switches_Melanoma_ensps[,c(3,9)] %>% dplyr::filter(Protein.stable.ID %in% Main_output_filtered_dMDT_exc_MDT_exc_ClassI_sepRows$Protein.Ids) %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()
...
mel_exp_matrix <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Melanoma/TPMValues/RedundantTPMs/tuPro_tpm_NR.RDS')
...
for (each_sample in common_samples) {
    ...
    dMDT_without_evi_dia <- mel_exp_matrix[mel_exp_matrix$ENST %in% ensg_enst_ensp_apris_v104[ensg_enst_ensp_apris_v104$Protein.stable.ID %in% NonFoundMDP, 'Transcript.stable.ID'], c('ENST',colnames(mel_exp_matrix[grepl(each_sample, colnames(mel_exp_matrix))]))]
    

    dMDT_with_evi_dia <- mel_exp_matrix[mel_exp_matrix$ENST %in% ensg_enst_ensp_apris_v104[ensg_enst_ensp_apris_v104$Protein.stable.ID %in% foundMDT, 'Transcript.stable.ID'], c('ENST',colnames(mel_exp_matrix[grepl(each_sample, colnames(mel_exp_matrix))]))]
...
```{r splitNames}
Switches_Melanoma_ensps$RunID <- do.call(rbind, strsplit(Switches_Melanoma_ensps$SampleID, '/'))[,2]
...
rna_probabilit_proteomics_transcript <-  Switches_Melanoma_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::filter(RunID %in% common_samples) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n)) %>% dplyr::filter(Protein.stable.ID %in% final_data$peptides) %>% ungroup %>% dplyr::select(Protein.stable.ID, n) %>% dplyr::distinct()
...
probabilit_proteomics_84_transcript <- final_data_allcMDTs[,c(1,4)] %>% dplyr::distinct() %>% dplyr::group_by(peptides) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n)) %>% dplyr::select(peptides, n) %>% dplyr::distinct()


Swicthes_in_Dataset_proteomics <- Swicthes_in_Dataset_gtex_tp_mel[Swicthes_in_Dataset_gtex_tp_mel$Protein.stable.ID %in% Main_output_filtered_dMDT_exc_MDT_exc_ClassI_sepRows$Protein.Ids, ]


rna_probabilit_proteomics_84_transcript <- Switches_Melanoma_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::filter(RunID %in% common_samples) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n)) %>% dplyr::filter(Protein.stable.ID %in% final_data_allcMDTs$peptides) %>% dplyr::ungroup() %>% dplyr::select(Protein.stable.ID, n) %>% dplyr::distinct()


probabilit_proteomics_84_transcript_ordered <- probabilit_proteomics_84_transcript[match(rna_probabilit_proteomics_84_transcript$Protein.stable.ID, probabilit_proteomics_84_transcript$peptides),]
```


```{r APRIS_annotations}
# APRIS annotation distribution of dMDTs with non-MDT peptide evidence. 
probabilit_proteomics_transcript_ordered_apris <- merge(probabilit_proteomics_84_transcript_ordered, ensg_enst_ensp_apris_v104, by.x='peptides', by.y='Protein.stable.ID')
...
```{r saveAPRIS}
apris_Melanoma <- ggplot(data=probabilit_proteomics_transcript_ordered_apris_dMDTpep, aes(APPRIS.annotation)) +
  geom_bar(color = "black", fill= "#C51B7D") +
  theme_classic()  + scale_fill_brewer(palette="Paired")

apris_Melanoma_2 <- ggplot(data=probabilit_proteomics_transcript_ordered_apris, aes(APPRIS.annotation, color=APPRIS.annotation)) +
...
#ggsave("/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Figures/apris_Melanoma_41E#NST.png", plot = apris_Melanoma, bg = "transparent", width = 5, height = 4)
#ggsave("/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Figures/apris_Melanoma_100#ENST.png", plot = apris_Melanoma_2, bg = "transparent", width = 5, height = 4)
```
...
rna_probabilit_proteomics_84_transcript$Group <- 'RNA'
colnames(rna_probabilit_proteomics_84_transcript) <- c('peptides', 'n', 'Group')
rna_probabilit_proteomics_84_transcript$n2 <- rna_probabilit_proteomics_84_transcript$n/length(unique(common_samples))*100

probabilit_proteomics_84_transcript_ordered$Group <- 'Proteomics_Flex'
probabilit_proteomics_84_transcript_ordered$n2 <- probabilit_proteomics_84_transcript_ordered$n/length(unique(final_data_allcMDTs$sample))*100

probabilit_proteomics_84_transcript_selected <- probabilit_proteomics_84_transcript_ordered[probabilit_proteomics_84_transcript_ordered$peptides %in% probabilit_proteomics_transcript_ordered$peptides,]

probability_all <- rbind(probabilit_proteomics_transcript_ordered, probabilit_proteomics_84_transcript_selected, rna_probabilit_proteomics_transcript)
...
set.seed(43)
probabilit_proteomics_84_transcript_uniq <- probabilit_proteomics_84_transcript_ordered[!probabilit_proteomics_84_transcript_ordered$peptides %in% probabilit_proteomics_transcript_ordered$peptides,]

probability_flex <- rbind(probabilit_proteomics_84_transcript_uniq,rna_probabilit_proteomics_84_transcript)
...
```{r checkExpression}
Switches_Melanoma_ensps_top_proteins <- Switches_Melanoma_ensps[Switches_Melanoma_ensps$Protein.stable.ID %in% probabilit_proteomics_transcript_ordered$peptides,] %>% dplyr::group_by(Protein.stable.ID) %>% dplyr::summarise(meanTPMinCancer = mean(TPM1_cancer)) %>% dplyr::arrange(desc(meanTPMinCancer))

head(Switches_Melanoma_ensps_top_proteins)
```


```{r graphHighlyExpressedOnes}
probability_flex_top20_exp <-  Switches_Melanoma_ensps_top_proteins[1:20,]$Protein.stable.ID
...
## TCGA data check 
```{r read_cMDT_TCGA}
Switches_Melanoma_tcga1 <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Melanoma/Switches/Switches_TCGA_Pri_GTEx.RDS')
Switches_Melanoma_tcga2 <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Melanoma/Switches/Switches_TCGA_Met_GTEx.RDS')
Switches_Melanoma_tcga <- rbind(Switches_Melanoma_tcga1, Switches_Melanoma_tcga2)
```

```{r checkTCGA}
# Class II peptide
unique(Switches_Melanoma_tcga[Switches_Melanoma_tcga$dMDT %in% probabilit_proteomics_transcript_ordered_apris$Transcript.stable.ID,  'dMDT'])

## Class I peptide
unique(Switches_Melanoma_tcga[Switches_Melanoma_tcga$dMDT %in% probabilit_proteomics_transcript_ordered_apris_dMDTpep$Transcript.stable.ID, 'dMDT'])
```

Switches_Melanoma_tcga[Switches_Melanoma_tcga$dMDT %in% probabilit_proteomics_transcript_ordered_apris$Transcript.stable.ID, ] %>% dplyr::group_by(dMDT) %>% dplyr::summarise(count = n()) %>% dplyr::arrange(desc(count))
