...
```{r read_output_diann}
Main_output <- read_delim('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Proteomics/AML/DIANN/Main_Report.tsv', delim = '\t', col_names = TRUE)
head(Main_output)
...
```{r read_MDTs}
Switches_AML <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/AML/Switches/CorrectedStat/Switches_TuPro_GTEx.RDS')
```
...
Main_output_filtered_dMDT <- Main_output_filtered[Main_output_filtered$Protein.Ids %in% Switches_AML_ensps$Protein.stable.ID, ]
...
Main_output_filtered_dMDT_exc_MDT_exc_ClassI_sepRows <- Main_output_filtered_dMDT_exc_MDT_exc_ClassI %>% separate_rows(Protein.Ids, sep = ";") %>% dplyr::filter(Protein.Ids %in% Switches_AML_ensps$Protein.stable.ID) %>% dplyr::filter(!Protein.Ids %in% Main_output_filtered_dMDT$Protein.Ids)
```


```{r focus_only_on_unique_peptides}
dMDT_proteomics_dt_unique_peptide <- dMDT_proteomics_dt[dMDT_proteomics_dt$Protein.Ids %in% Switches_AML_ensps$Protein.stable.ID, ] # 121 - dMDT specific peptides are counted.

MDT_proteomics_dt_unique_peptide <- MDT_proteomics_dt[MDT_proteomics_dt$Protein.Ids %in% Switches_AML_ensps_mdts$Protein.stable.ID, ] # unique MDT peptides - 311 MDT peptides are found in the samples. 

Swicthes_in_Dataset_gtex_tp_mel <- Switches_AML_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n))
```
...
#rna_each_sample_output <- Switches_AML_ensps[,c(3,9)] %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()

## Class I isoforms in the RNA Samples
rna_each_sample_output_3 <- Switches_AML_ensps[,c(3,9)] %>% dplyr::filter(Protein.stable.ID %in% Main_output_filtered_dMDT$Protein.Ids) %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()

## Class II isoforms in the RNA samples
rna_each_sample_output_4 <- Switches_AML_ensps[,c(3,9)] %>% dplyr::filter(Protein.stable.ID %in% Main_output_filtered_dMDT_exc_MDT_exc_ClassI_sepRows$Protein.Ids) %>% dplyr::group_by(SampleID) %>% dplyr::mutate(Protein_IDS_in_Sample = paste(Protein.stable.ID, collapse = ',')) %>% dplyr::select(SampleID, Protein_IDS_in_Sample) %>% dplyr::distinct()
...
aml_exp_matrix <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/AML/TPMValues/RedundantTPMs/tuPro_tpm_NR.RDS')
...
for (each_sample in common_samples) {
    ...
    dMDT_without_evi_dia <- aml_exp_matrix[aml_exp_matrix$ENST %in% ensg_enst_ensp_apris_v104[ensg_enst_ensp_apris_v104$Protein.stable.ID %in% NonFoundMDP, 'Transcript.stable.ID'], c('ENST',colnames(aml_exp_matrix[grepl(each_sample, colnames(aml_exp_matrix))]))]
    

    dMDT_with_evi_dia <- aml_exp_matrix[aml_exp_matrix$ENST %in% ensg_enst_ensp_apris_v104[ensg_enst_ensp_apris_v104$Protein.stable.ID %in% foundMDT, 'Transcript.stable.ID'], c('ENST',colnames(aml_exp_matrix[grepl(each_sample, colnames(aml_exp_matrix))]))]
...
```{r splitNames}
Switches_AML_ensps$RunID <- do.call(rbind, strsplit(Switches_AML_ensps$SampleID, '/'))[,2]
...
rna_probabilit_proteomics_transcript <-  Switches_AML_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::filter(RunID %in% common_samples) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n)) %>% dplyr::filter(Protein.stable.ID %in% final_data$peptides) %>% ungroup %>% dplyr::select(Protein.stable.ID, n) %>% dplyr::distinct()
...
rna_probabilit_proteomics_84_transcript <- Switches_AML_ensps %>% dplyr::group_by(Transcript.stable.ID) %>% dplyr::filter(RunID %in% common_samples) %>% dplyr::summarize(n=n(), across()) %>% arrange(desc(n)) %>% dplyr::filter(Protein.stable.ID %in% final_data_allcMDTs$peptides) %>% dplyr::ungroup() %>% dplyr::select(Protein.stable.ID, n) %>% dplyr::distinct()
...
```{r saveAPRIS}
apris_AML <- ggplot(data=probabilit_proteomics_transcript_ordered_apris_dMDTpep, aes(APPRIS.annotation)) +
  geom_bar(color = "black", fill= "#C51B7D") +
  theme_classic()  + scale_fill_brewer(palette="Paired")

apris_AML_2 <- ggplot(data=probabilit_proteomics_transcript_ordered_apris, aes(APPRIS.annotation, color=APPRIS.annotation)) +
...
#ggsave("/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Figures/apris_AML_41E#NST.png", plot = apris_AML, bg = "transparent", width = 5, height = 4)
#ggsave("/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/Figures/apris_AML_100#ENST.png", plot = apris_AML_2, bg = "transparent", width = 5, height = 4)
```
...
```{r checkExpression}
Switches_AML_ensps_top_proteins <- Switches_AML_ensps[Switches_AML_ensps$Protein.stable.ID %in% probabilit_proteomics_transcript_ordered$peptides,] %>% dplyr::group_by(Protein.stable.ID) %>% dplyr::summarise(meanTPMinCancer = mean(TPM1_cancer)) %>% dplyr::arrange(desc(meanTPMinCancer))

head(Switches_AML_ensps_top_proteins)
```


```{r graphHighlyExpressedOnes}
probability_flex_top20_exp <-  Switches_AML_ensps_top_proteins[1:20,]$Protein.stable.ID
...
## TCGA data check 
```{r read_cMDT_TCGA}
Switches_AML_tcga <- readRDS('/Users/tulaykarakulak/Documents/PhD/Projects/TumorProfiler/Analysis/May2024/AML/Switches/Switches_TCGA_GTEx.RDS')
```

```{r checkTCGA}
# Class II peptide
Switches_AML_tcga[Switches_AML_tcga$dMDT %in% probabilit_proteomics_transcript_ordered_apris$Transcript.stable.ID, ]

## Class I peptide
unique(Switches_AML_tcga[Switches_AML_tcga$dMDT %in% probabilit_proteomics_transcript_ordered_apris_dMDTpep$Transcript.stable.ID, 'dMDT'])
```
